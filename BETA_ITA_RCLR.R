...
#Aitchison distance
general.tse <- makeTreeSummarizedExperimentFromPhyloseq(ps1)

#transform counts  by clr transformation
general.tse.clr <- transformCounts(general.tse, method = "rclr", abund_values = "counts")

#view clr table
general.clr_assay <- assays(general.tse.clr)$rclr

#transpose clr table
general.clr_assay <- t(general.clr_assay)

#calcualte Euclidean distance between samples
general.euclidean_dist <- vegan::vegdist(general.clr_assay, method="euclidean")

#PERMANOVA
#getting relative abundance
general.tax_abundance <- assays(general.tse.clr)$rclr
general.tax_abundance <- t(general.tax_abundance)

general.pca <- prcomp(general.clr_assay, center=T, scale = F)

#PCA result for variables
var.res.general <- get_pca(general.pca, "var")

#screeplot
general.sd <- general.pca$sdev
general.loadings <- general.pca$rotation
rownames(general.loadings) <- colnames(general.euclidean_dist)
general.scores <- general.pca$x
general.var <- general.sd^2
general.varPercent <- (general.var/sum(general.var))*100

general.pca.scree <- data.frame(general.sd, general.var, general.varPercent)

ggplot(general.pca.scree,
       aes(x = 1:nrow(general.pca.scree), y=general.varPercent)) +
  geom_col() +
  geom_hline(yintercept = (1/nrow(general.pca.scree))*100,
...
ggsave(plot = last_plot(),
       filename = "PCA.ita.general.scree.pdf",
       device = "pdf",
       path = "Final_figures/",
...
#explained variance
sum(general.varPercent[1:31]) #60.15847% contribution of the first five PCs

general.loadings <- general.pca$rotation # Loadings Matrix
general.scores <- predict(general.pca) # Scores Matris
general.importance <- summary(general.pca)$importance # Explained Variance

# The plot data includes occupation and the first two PC components.
general.plotDataScores <- cbind(general.scores[, c("PC1", "PC2")],
                                colData(general.tse.clr)) %>%
  as.data.frame()

# We want to use min-max feature scaling on the scores so they are between zero and one, the same as the loadings.
normalize <- function(x) return ((x - min(x)) / (max(x) - min(x)))

general.plotDataScores[, "PC1"] <- scale(normalize(general.plotDataScores[, "PC1"]), center = TRUE, scale = FALSE)
general.plotDataScores[, "PC2"] <- scale(normalize(general.plotDataScores[, "PC2"]), center = TRUE, scale = FALSE)

general.plotDataLoadings <- as.data.frame(general.loadings) %>%
  filter(row.names(general.plotDataLoadings) %in% c("ASV2113", "ASV1642", "ASV2772", "ASV515", "ASV2132",
                                                    "ASV1", "ASV138", "ASV2129", "ASV2134", "ASV302")) %>%
  select(c(PC1, PC2))

ggplot(general.plotDataScores, aes(x = PC1[,1], y = PC2[,1])) +
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
  geom_vline(xintercept = 0, linetype = "dashed", alpha = 0.5) +
  geom_point(data = general.plotDataScores, aes(fill = soil, shape = compartment), size = 3, alpha = 1, color = "black") +
  geom_segment(data = general.plotDataLoadings, aes(x = 0, y = 0, xend = PC1, yend = PC2),
               arrow = arrow(length = unit(0.03, "npc")), alpha = 0.5) + 
  geom_text(data = general.plotDataLoadings,
            mapping = aes(x = PC1, y = PC2,label = rownames(general.plotDataLoadings)),
            hjust = 1, vjust = -0.2, colour = "darkred", size = 4, check_overlap = FALSE) +
  scale_fill_manual(values=c("#E3AC37", "#376EAA")) +
  scale_shape_manual(values = c(21, 24, 22)) +
  guides(fill = guide_legend(override.aes=list(shape=21))) +
  xlab("PC1 (27.8% explained variance)") +
  ylab("PC2 (3.8% explained variance)") +
  theme_bw() +
  theme(panel.background = element_rect(fill = "white"),
        plot.background = element_blank(),
        panel.grid.major = element_blank(),
...
ggsave(plot = last_plot(),
       filename = "PCA.general.pdf",
       device = "pdf",
       path = "Final_figures/",
       width = 2100, height = 1475, unit = "px",
       dpi = 300)

general.var.dat <- fviz_contrib(general.pca, choice = "var", axes = 1:2, top = 25) +
  ...
  scale_x_discrete(labels = c(
    ...
    "ASV1: Methylocystis",
    "ASV138: Unclassified Methylocystis echinoides",
    "ASV2129: Methylococcus",
    "ASV2134: Methylococcus",
    "ASV302: Methylocystis",
    "ASV964: Methylocystis",
    "ASV1621: RPC 2d",
    "ASV2320: Methylococcus",
    "ASV2459: Methylocystis",
    "ASV2139: Methylococcus",
    "ASV2843: Methylocystis",
    "ASV922: Methylocystis",
    "ASV2722: Methylocystis",
    "ASV160: Unclassified Methylocystaceae",
    "ASV2135: Methylococcus",
    "ASV2476: Methylocystis",
    "ASV854: Methylocystis",
    "ASV2484: Methylocystis",
    "ASV38: Methylocystis",
...
ggsave(plot = last_plot(),
       filename = "PCA.ita.general.contrib.pdf",
       device = "pdf",
       path = "Final_figures/",
...
#Italian soil subset
phylo.ita <- subset_samples(ps1, soil == "Italian paddy soil" & 
                              compartment %in% c("Rhizosphere", "Bulk soil"))

#Aitchison distance
rhizosphere.tse <- makeTreeSummarizedExperimentFromPhyloseq(phylo.ita)
...
ggsave(plot = last_plot(),
       filename = "PCA.ita.rhizosphere.scree.pdf",
       device = "pdf",
       path = "Final_figures/",
       width = 2000, height = 1000, unit = "px",
       dpi = 300)
...
rhizosphere.plotDataLoadings <- as.data.frame(rhizosphere.loadings) %>%
  filter(row.names(rhizosphere.loadings) %in% c("ASV1642", "ASV2113", "ASV1621", "ASV2132", "ASV2320",
                                                "ASV2134", "ASV1910", "ASV2129", "ASV2772", "ASV1")) %>%
  select(c(PC1, PC2))
...
ggsave(plot = last_plot(),
       filename = "PCA.ita.rhizosphere2.pdf",
       device = "pdf",
       path = "Final_figures/",
...
var.dat <- fviz_contrib(rhizosphere.pca, choice = "var", axes = 1:2, top = 25) +
  ...
  scale_x_discrete(labels = c(
    ...
    "ASV2139: Methylococcus",
    "ASV62: Methylocystis",
    "ASV537: Unclassified Methylocystaceae",
    "ASV2322: Methylococcus",
    "ASV2135: Methylococcus",
    "ASV589: Unclassified Methylocystaceae",
    "ASV964: Methylocystis",
    "ASV753: Methylocystis",
    "ASV938: Methylocystis",
    "ASV647: Methylocystis",
    "ASV2465: Methylocystis",
    "ASV649: Methylocystis",
    "ASV922: Methylocystis"
  )) +
  theme_bw() +
  theme(panel.background = element_rect(fill = "white"),
...
ggsave(plot = last_plot(),
       filename = "PCA.ita.rhizosphere.contrib.pdf",
       device = "pdf",
       path = "Final_figures/",
       width = 2000, height = 1500, unit = "px",
       dpi = 300)
...
#Root subset
phylo.ita.root <- subset_samples(ps1, soil == "Italian paddy soil" & 
                                   compartment %in% c("Root", "Bulk soil"))

#Aitchison distance
Root.tse <- makeTreeSummarizedExperimentFromPhyloseq(phylo.ita.root)
...
ggsave(plot = last_plot(),
       filename = "PCA.ita.root.scree.pdf",
       device = "pdf",
       path = "Final_figures/",
       width = 2000, height = 1000, unit = "px",
       dpi = 300)
...
#explained variance
sum(root.varPercent[1:17]) #62.31579% contribution of the first five PCs
...
Root.plotDataLoadings <- as.data.frame(Root.loadings) %>%
  filter(row.names(Root.loadings) %in% c("ASV2113", "ASV1642", "ASV371", "ASV375",
                                         "ASV2320", "ASV302", "ASV2129", "ASV964", "ASV2139", "ASV2132")) %>%
  select(c(PC1, PC2))
...
ggsave(plot = last_plot(),
       filename = "PCA.ita.root.pdf",
       device = "pdf",
       path = "Final_figures/",
       width = 1500, height = 2000, unit = "px",
       dpi = 300)

var.dat <- fviz_contrib(root.pca, choice = "var", axes = 1:2, top = 25) +
  xlab("Amplicon sequence variants") +
  scale_x_discrete(labels = c(
    "ASV2113: Methylococcus",
    "ASV1642: RPC 2a",
    "ASV371: Methylocystis",
    "ASV375: Methylocystis",
    "ASV2320: Methylococcus",
    "ASV302: Methylocystis",
    "ASV2129: Methylococcus",
    "ASV964: Methylocystis",
    ...
    "ASV2134: Methylococcus",
    "ASV1621: RPC 2d",
    "ASV2513: Methylocystis",
    "ASV2327: Methylococcus",
    "ASV2330: Methylococcus",
    "ASV2328: Methylococcus",
    "ASV38: Methylocystis",
    "ASV2135: Methylococcus",
    "ASV515: Unclassified Methylocystaceae",
    "ASV891: Methylocystis",
    "ASV2467: Methylocystis",
    "ASV2843: Methylocystis",
    "ASV2489: Methylocystis",
    "ASV366: Methylocystis")) +
  theme_bw() +
  theme(panel.background = element_rect(fill = "white"),
        plot.background = element_blank(),
        panel.grid.major = element_blank(),
...
ggsave(plot = last_plot(),
       filename = "PCA.ita.root.contrib.pdf",
       device = "pdf",
       path = "Final_figures/",
       width = 2000, height = 1500, unit = "px",
       dpi = 300)

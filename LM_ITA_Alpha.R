...
phylo2 = subset_samples(phylo, soil=="Italian paddy soil" & 
...
#post-hoc test
rhizosphere.lmer.posthoc <- cld(emmeans(lm.rhizosphere, c("genotype", "nitro")), alpha = 0.05, Letters = "letters")
...
ggsave(plot = last_plot(),
       filename = "Alpha.ita.rhizosphere.pdf",
       device = "pdf",
       path = "final_figures/",
...
#Root subset
phylo3 = subset_samples(phylo, soil=="Italian paddy soil" & 
...
#post-hoc test
root.lmer.posthoc <- cld(emmeans(lm.root, c("genotype", "nitro")), alpha = 0.05, Letters = "letters")
